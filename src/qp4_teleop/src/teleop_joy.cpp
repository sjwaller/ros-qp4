...
TeleopJoy::TeleopJoy(){
	...
	move_body_pub 	= node.advertise<qp4_msgs::BodyState>("/teleop/move_body",1);
	body_cmd_pub 	= node.advertise<qp4_msgs::BodyCommand>("/teleop/body_command",1);
	gait_cmd_pub 	= node.advertise<qp4_msgs::GaitCommand>("/teleop/gait_control",1);
}
...
void TeleopJoy::joyCallback(const sensor_msgs::Joy::ConstPtr& joy){
	...
	if (joy->buttons[button_start] && !imu_flag){
		if (!start_flag){
			...
			start_flag = false;
			body_command.cmd = body_command.SIT_DOWN_CMD;
			body_cmd_pub.publish(body_command);
		}
